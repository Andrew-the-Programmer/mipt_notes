...
3. $\xi = \eta\ (\textit{почти наверное - п.н. - P почти всюду})
\implies E(\xi) = E(\eta)$
4. $\xi \le \eta\ (п.н.) \implies E(\xi) \le E(\eta)$
5. $\xi \ge 0\ (п.н.),\quad E(\xi) \ge 0 \implies \xi = 0\ (п.н.)$
6. $X,Y$ - НСВ $\implies E(XY) = E(X)E(Y)$
...
3. $$
D \groupr{\sum{c_i X_i}} = \sum{c_i^2 D X_i}
$$
