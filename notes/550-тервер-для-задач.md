...
Be(p) & p & pq
\\ \hline
Биномиальное $Binom(n, p)$ & $np$ & $npq$ \\ \hline
Геометрическое $Geom(p)$ & $\frac{q}{p}$ & $\frac{q}{p^2}$ \\ \hline
Пуассона $Poiss(\lambda)$ & $\lambda$ & $\lambda$ \\ \hline
Равномерное $U(a, b)$ & $\frac{a + b}{2}$ & $\frac{(b - a)^2}{12}$ \\ \hline
Нормальное $\mathcal{N}(m, \sigma^2)$ & $m$ & $\sigma^2$ \\ \hline
Гамма $Gamma(\alpha, \lambda)$ & $\frac{\alpha}{\lambda}$ & $\frac{\alpha}{\lambda^2}$ \\ \hline
Показательное $Exp(\lambda)$ & $\frac{1}{\lambda}$ & $\frac{1}{\lambda^2}$ \\ \hline
Бета $Beta(\alpha, \beta)$ & $\frac{\alpha}{\alpha + \beta}$ & $\frac{\alpha \beta}{(\alpha + \beta)^2 (\alpha + \beta + 1)}$ \\ \hline
Коши $\mathcal{K}(m, \sigma)$ & — & — \\ \hline
}
$$
