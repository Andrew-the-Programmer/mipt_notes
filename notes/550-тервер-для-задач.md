...
Gamma(\alpha, \lambda) & $\left(\frac{\lambda}{\lambda - i t}\right)^{\alpha}$ \\ \hline
\chi^2(n) & $\left(\frac{\frac{1}{2}}{\frac{1}{2} - i t}\right)^{\frac{n}{2}}$ \\ \hline
Exp(\lambda) & $\frac{\lambda}{\lambda - i t}$ \\ \hline
Beta(\alpha, \beta) & \text{Нет в явном виде (через интеграл)} \\ \hline
\mcK(m, \sigma) & $e^{-|t|}$ \\ \hline
}
$$
